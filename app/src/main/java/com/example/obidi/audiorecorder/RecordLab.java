...
public class RecordLab {
    ...
    public List<Recording> getRecordings() {
        ...
        ArrayList<RecordLab.Recording> recordings = new ArrayList<>();
        ...
        for (int i = fileList.length - 1; i >= 0; i-- ) {
            File f = fileList[i];
            if (!f.isDirectory()) {
                RecordLab.Recording recording = new RecordLab.Recording(f);
    ...
    private static class NameComparator implements Comparator<Recording> {
        @Override
        public int compare(RecordLab.Recording obj1, RecordLab.Recording obj2) {
            return obj1.getName().compareToIgnoreCase(obj2.getName());
        }
    }

    // Comparator Class for sorting recordings according to their dates
    private static class RecentComparator implements Comparator<RecordLab.Recording> {
        @Override
        public int compare(RecordLab.Recording obj1, RecordLab.Recording obj2) {
